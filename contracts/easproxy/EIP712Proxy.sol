...
contract EIP712Proxy is EIP712 {
    ...
    mapping(address => mapping(bytes32 schemaid => bytes32[])) internal _padoAttestations;
    ...
    function bulkAttest(
        ...
        bytes32[] memory uids = _eas.multiAttest(multiRequests);
        ...
        for (uint256 i = 0; i < multiDelegatedRequests.length; i = uncheckedInc(i)) {
            ...
            for (uint256 j = 0; j < data.length; j = uncheckedInc(j)) {
                ...
                _padoAttestations[data[j].recipient][multiDelegatedRequest.schema].push(uids[uidCounter]);
