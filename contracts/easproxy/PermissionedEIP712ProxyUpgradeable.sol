...
} from "./EIP712ProxyUpgradeable.sol";

import { IEAS } from "@ethereum-attestation-service/eas-contracts/contracts/IEAS.sol";

import { AccessDenied, uncheckedInc, Attestation } from "@ethereum-attestation-service/eas-contracts/contracts/Common.sol";

/**
 * @title A sample EIP712 proxy that allows only a specific address to attest.
 */
contract PermissionedEIP712ProxyUpgradeable is EIP712ProxyUpgradeable, OwnableUpgradeable {
    ...
    function initialize(IEAS eas, string memory name, uint256 feeParams, address payable recvAddr, bytes32 webSchemaId) external initializer {
        super.initialize(eas, name);
        __Ownable_init();
         _fee = feeParams;
    ...
    /**
     * @inheritdoc EIP712ProxyUpgradeable
     */
    ...
    function bulkAttest(
        ...
        for (uint256 i = 0; i < multiDelegatedRequests.length; i = uncheckedInc(i)) {
            ...
            if (!eventSchemas[multiDelegatedRequests[i].schema]) {
                _padoAttestations[multiDelegatedRequests[i].data.recipient][multiDelegatedRequests[i].schema].push(uid);
            }
        }
    }
    ...
    function checkCommon(address userAddress, string memory proofType, string memory source, string memory content, string memory condition) public view returns (bool) {
        ...
        for (uint256 i = 0; i < attestationIds.length; i++) {
            Attestation memory ats = _eas.getAttestation(attestationIds[i]);
